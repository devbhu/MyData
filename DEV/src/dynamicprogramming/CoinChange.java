...
public class CoinChange
{
	 ...
	 * @author Tushar Roy
	 * 
	 *         Given a total and coins of certain denomination with infinite
	 *         supply, what is the minimum number of coins it takes to form this
	 *         total.
	 * 
	 *         Time complexity - O(coins.size * total) Space complexity -
	 *         O(coins.size * total)
	 * 
	 *         Youtube video - Topdown DP - https://youtu.be/Kf_M7RdHr1M Bottom
	 *         up DP - https://youtu.be/Y0ZqKpToTic
	 */
	 ...
	 */
	// Non dynamic code to find mininmum no of coins
	public int minimumCoinTopDownNonDynamic(int total, int coins[])
	{
		...
		for (int i = 0; i < coins.length; i++)
		{
			...
			int val = minimumCoinTopDownNonDynamic(total - coins[i], coins);
	...
	public int minimumCoinTopDown(int total, int coins[],
	...
	public static void main(String args[])
	{
		...
		int topDownValueNonDynamic = cc.minimumCoinTopDownNonDynamic(total,
				coins);
		System.out
				.print(String
						.format("Bottom up , top down  and non dynamic top down result %s %s %s",
								bottomUpValue, topDownValue,
								topDownValueNonDynamic));
